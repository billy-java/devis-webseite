...
import Modal_Webseite from '@/app/components/Modals/Modal_Webseite';
import { I_Webseite } from '@/app/lib/Interfaces/I_Webseite';
import { useDispatch, useSelector } from 'react-redux';
import { deleteWebseite__Hilfe } from '@/app/services/Reduces/webseiten_Slice';
import { date_to_french_Format } from '@/app/lib/Methodes/methodes';
...
const Webseiten = () => {
  ...
  useEffect(() => {
    if (!currentUser) {
      ...
      if (aktuelUser) {
        setUser(aktuelUser);
        if (aktuelUser.status === 'ADMIN') {
          const newConf = { ...config, admin: true };
          setConfig(newConf);
        }
      }
    }
  }, [config, currentUser, router, usersList]);

  const webseiten = useSelector((state: RootState) => state.webseiten);
  const meineWebseiten = config.admin
    ? webseiten
    : webseiten.filter((el) => el.user_id === user?.id);
  ...
  const handleEdit = (webseiteOption: I_Webseite) => {
    const newConf = {
      ...config,
      openModal: true,
      edit: true,
      wbSeite: webseiteOption,
    };
    setConfig(newConf);
  };

  const handleDelete = (id: string) => {
    dispatch(deleteWebseite__Hilfe(id));
  };
  ...
  return (
    <div className="h-full min-h-screen w-full  pt-12 p-4">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3">
        {meineWebseiten.map((oneWebseite, index) => {
          return (
            <div
              key={index}
              ...
              <div>
                <div className="mx-auto flex h-16 w-16 -translate-y-12 transform items-center justify-center rounded-full bg-teal-400 shadow-lg shadow-teal-500/40">
                  <svg
                    className="w-10 h-10 text-gray-800 dark:text-white" // Larger size using Tailwind classes
                    aria-hidden="true"
                ...
                <h1 className="uppercase text-gray-300 mb-3 text-xl font-medium lg:px-14">
                  {oneWebseite.title_WB}
                </h1>
                <p className="px-4 text-gray-100">{oneWebseite.slogan}.</p>
                <p className="px-4 text-gray-100">
                  {oneWebseite.page_Anzahl + ' Seiten.'}
                </p>
                {user && user.status === 'ADMIN' ? (
                  <>
                    ...
                    <p className="px-4 text-gray-100">
                      {webseiten[index].anprechPartner}
                    </p>
                    <p className="px-4 text-gray-100">
                      Tel:{webseiten[index].tel_anPart}
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="px-4 text-gray-100">Billy</p>
                    <p className="px-4 text-gray-100">Tel: +49 30 9876543</p>
                  </>
                )}
                ...
                <p className="px-4 text-gray-100">
                  {date_to_french_Format(oneWebseite.liefert_Datum)}
                </p>

                <p className="px-4 mb-1 mt-4 font-bold text-gray-400">
                  Schrift, Farben und Tel der Seite
                </p>
                <p className="px-4 text-gray-100">
                  Schrift: {oneWebseite.schrift}.
                </p>
                <p className="px-4 text-gray-100">
                  {oneWebseite.farbe1} , {oneWebseite.farbe1}
                </p>
                <p className="px-4 text-gray-100">Tel: {oneWebseite.tel_WB}</p>
                <p className="px-4 mt-4 font-semibold text-cyan-500">
                  {oneWebseite.type}
                </p>
                <p className="px-4 mb-4 text-gray-100">
                  www.{oneWebseite.domain}
                </p>
              </div>
              ...
              <div>
                <div className="flex justify-center gap-4 mt-4">
                  <button
                    onClick={() => handleEdit(oneWebseite)}
                  ...
                  {config.admin && (
                    <button
                      onClick={() => handleDelete(oneWebseite.id)}
                      className={`p-2 rounded-full group transition-all duration-500 flex items-center bg-red-600 hover:bg-gray-300`}>
                      <svg
                        className=""
                        width="20"
                        height="20"
                  ...
                  <Link
                    href={'https://' + oneWebseite.img_Folder}
                    target="_blank"
                    className={`p-2 rounded-full group transition-all duration-500 flex items-center bg-blue-500 hover:bg-blue-700 hover:border hover:border-gray-300`}>
                    <p className="rounded-full text-gray-300   group transition-all duration-500 flex item-center">
                      Seiten
                    </p>
                  </Link>
                </div>
                <p className="p-6 text-gray-100">
                  <a
                    target="_blank"
                    href={oneWebseite.img_Folder}
                    className="px-6 py-2 bg-indigo-600 hover:bg-indigo-800 text-white font-bold  rounded-full transition duration-300 ease-in-out inline-block">
                    Die Bilder ansehen
                  </a>
                </p>
                <p className="mt-4 text-gray-300 text-sm">
                  Seiteid:{oneWebseite.id}
                </p>
              </div>
              ...
              {config.openModal && (
                <Modal_Webseite
                  ...
                  alteWebseite={config.edit ? config.wbSeite : undefined}
                />
              )}
            </div>
          );
