...
const Webseiten = () => {
  ...
  const [boolUpdate_Parent, setBoolUpdate_Parent] = useState(false);
  const [webseiteEditer, setWebseiteEditer] = useState<I_Webseite | null>(null);
          ...
          .map((oneWebseite, index) => {
            return (
              <div
                  ...
                  <div className="flex justify-center gap-4 mt-4">
                    <button
                      onClick={() => {
                        ...
                        setBoolUpdate_Parent(true); // Indique qu'il s'agit d'une mise à jour
                        setWebseiteEditer(oneWebseite); // Transmet la bonne carte à modifier
                      }}
                    ...
                    <button
                      onClick={() =>
                        dispatch(deleteWebseite__Hilfe(oneWebseite.id))
                      }
                      className={`p-2 rounded-full group transition-all duration-500 flex items-center ${
                        user && user.status === 'ADMIN'
                          ? 'bg-red-600 hover:bg-gray-300'
                          : 'bg-blue-500 hover:bg-blue-700 hover:border hover:border-gray-300'
                      }`}>
                      {user && user.status === 'ADMIN' ? (
                        <svg
                          className=""
                ...
                <Modal_Webseite
                  isOpen={modalOpen}
                  boolUpdate={boolUpdate_Parent}
                  alteWebseite={boolUpdate_Parent ? webseiteEditer : null}
                  userID={user!.id}
                  onClose={() => {
                    setModalOpen(false);
                    setBoolUpdate_Parent(false);
